...
Class 97 (TOWER_TYCOON) now carries **adult Champion Silver** (source art
`assets/src/trainers/front/silver_adult_front.png`, 6-frame strip; design:
Palmer's arms-crossed longcoat body + vanilla class-23 head transplant,
charcoal-indigo coat, crimson shoulder mantle). Tradeoff: vanilla Palmer
loses his sprite — the Battle Frontier tycoon fights would show Silver if
ever reachable. Ch8 Champion-Silver trainer entries should use
`TRAINERCLASS_TOWER_TYCOON`. Class 97's aux NCGR is garbage/unused in
vanilla (decodes to noise) and was left as-is. Overworld: the GSRIVEL
walker (mmodel 58) was recolored **in place** to the adult design
(source `assets/src/trainers/overworld/silver_adult_ow.png`) — every
Silver map object shows the adult; walker palette index 8 is dual-used
in the hair, leave it untouched. Teen art recoverable from git and
`artwork-library/`.
