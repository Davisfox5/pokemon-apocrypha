#!/usr/bin/env python3
"""Import a reviewed HGSS NPC profile into an isolated Cherrygrove v3 checkout.
...
ART=ROOT/'gba/art/johto-npc-v1'
p=argparse.ArgumentParser();p.add_argument('game',type=Path);a=p.parse_args();game=a.game.resolve()
assert game==ROOT/'tools/vendor/gba/johto-npc-v1-game', 'Use the isolated NPC preview.'
...
def section(path,body):
    start='// BEGIN JOHTO NPC IMPORT';end='// END JOHTO NPC IMPORT'
...
for s in spec['assets']:
    ...
    source=ROOT/'artwork-library/heartgold-johto/overworld-sprites'/s['source']
    ...
    .subspriteTables = sOamTables_32x32, .anims = sAnimTable_JohtoHuman,
...
anims += ['static const union AnimCmd *const sAnimTable_JohtoHuman[] = {\n'+'\n'.join(table)+'\n};']
...
constants += ['#define OBJ_EVENT_GFX_JOHTO_FIRST 1024','#define OBJ_EVENT_GFX_JOHTO_END 1029',
              '_Static_assert(OBJ_EVENT_GFX_VAR_F < OBJ_EVENT_GFX_JOHTO_FIRST, "Johto graphics overlap dynamic IDs");',
              '_Static_assert(OBJ_EVENT_GFX_JOHTO_END < OBJ_EVENT_MON, "Johto graphics overlap follower IDs");']
...
replace(path,'const struct ObjectEventGraphicsInfo *const gObjectEventGraphicsInfoPointers[NUM_OBJ_EVENT_GFX] = {',
    '\n'.join(externs)+'\nconst struct ObjectEventGraphicsInfo *const gObjectEventGraphicsInfoPointers[OBJ_EVENT_GFX_JOHTO_END] = {\n'+'\n'.join(pointers))
...
replace('src/event_object_movement.c','    if (graphicsId >= NUM_OBJ_EVENT_GFX)',
        '    if (graphicsId >= OBJ_EVENT_GFX_JOHTO_FIRST && graphicsId < OBJ_EVENT_GFX_JOHTO_END)\n        return gObjectEventGraphicsInfoPointers[graphicsId];\n\n    if (graphicsId >= NUM_OBJ_EVENT_GFX)')
...
d.text((12,12),'JOHTO NPC IMPORT / actual converted pixels, 2x; S / N / W / E',fill='white')
...
d.text((490,80),'Original HGSS colors and pixels',fill='white')
d.text((490,105),'Fixed +1px vertical anchor',fill='white')
