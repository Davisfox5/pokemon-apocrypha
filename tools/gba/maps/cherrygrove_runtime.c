// Town-specific runtime checks reuse the qualified mGBA host call adapter.
...
int main(int argc,char**argv){
 ...
 if(!strcmp(argv[4],"menu")){
  ...
  go(0,41,19);frames(64,16);frames(40,0);expect(9,"route29-out");screenshot(argv[3],"route29");frames(80,32);frames(40,0);expect(0,"route29-return");
  go(0,15,2);frames(64,64);frames(40,0);expect(10,"route30-out");screenshot(argv[3],"route30");frames(80,128);frames(40,0);expect(0,"route30-return");
  go(0,22,37);frames(32,128);frames(20,0);expect(0,"pier-collision");if(state[4]!=37)return 10;
  go(0,10,18);frames(20,0);screenshot(argv[3],"residential-lane");
  go(0,9,8);screenshot(argv[3],"blossom-park");
  go(0,25,22);screenshot(argv[3],"gold-house");
  go(0,28,10);screenshot(argv[3],"center-and-mart");
  go(0,36,31);screenshot(argv[3],"lookout");
  go(0,19,32);frames(8,64);frames(8,0);frames(1,1);frames(200,0);screenshot(argv[3],"waterfront-dialogue");close_dialog();frames(60,0);
  go(5,3,3);frames(8,32);frames(8,0);frames(1,1);frames(150,0);frames(1,1);frames(90,0);screenshot(argv[3],"mart-menu");close_dialog();
  go(0,22,36);expect(0,"save-at-pier");screenshot(argv[3],"waterfront");unsigned status=call("TrySavingData",0);void*data=NULL;size_t size=c->savedataClone(c,&data);f=fopen(flash,"wb");if(status!=1||size!=131072||!f||fwrite(data,1,size,f)!=size)return 11;fclose(f);free(data);printf("{\"save_status\":%u,\"flash_bytes\":%zu}\n",status,size);
 }
 c->deinit(c);return 0;
}
