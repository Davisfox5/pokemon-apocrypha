...
assert game == ROOT/'tools/vendor/gba/regional-scale-v1-game'
out.mkdir(parents=True, exist_ok=True)
evidence = ROOT/'gba/art/regional-scale-v1/evidence'
if not a.reuse_recording:
    ...
    subprocess.run(['cc', '-I/opt/homebrew/opt/mgba/include', str(ROOT/'tools/gba/maps/regional_scale_runtime.c'), '-L/opt/homebrew/opt/mgba/lib', '-lmgba', '-o', str(out/'runtime')], check=True)
...
for row in rows:
    objects = row['objects']
    assert {o['gfx'] for o in objects} == set(range(1029,1037))
    assert len({o['palette'] for o in objects}) == 8
    assert len({(o['x'],o['y']) for o in objects}) == 8
    for o in objects:
        assert not o['hflip']
...
for gid, name, directions, positions in [
    ...
    (1032,'Hoenn',{1,2,3,4},{(44,9),(44,10),(45,9),(45,10)}),
    (1033,'Kanto',{1,2,3,4},{(47,9),(47,10),(48,9),(48,10)}),
    (1034,'Johto',{1,2,3,4},{(50,9),(50,10),(51,9),(51,10)}),
    (1035,'Sinnoh',{1,2,3,4},{(53,9),(53,10),(54,9),(54,10)}),
    (1036,'Unova',{1,2,3,4},{(56,9),(56,10),(57,9),(57,10)})]:
...
native = ROOT/'gba/art/regional-scale-v1/native'
for name in ['gold','silver','kestra','hoenn','kanto','johto','sinnoh','unova']:
    folder='johto' if name in ['gold','silver','kestra'] else 'regional'
    compiled = game/f'build/assets/graphics/object_events/pics/people/{folder}/{name}.png_mwidth_4__mheight_4.4bpp'
    palette = game/f'build/assets/graphics/object_events/palettes/{folder}_{name}.pal.gbapal'
...
report = dict(samples=len(rows), observed_engine_frames=1440, read_only_struct_probe=True,
              ...
              residents=summary, interaction=data[-8:])
...
run = ROOT/'tools/vendor/gba/regional-scale-v1-run'
...
for i,(label,file) in enumerate([
    ...
    ('Five regions and the smaller cast',out/'residents.ppm')]):
    x,y=i%2*480,i//2*350
