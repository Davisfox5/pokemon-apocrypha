#!/usr/bin/env python3
"""Register the three cleaned custom characters above the qualified NPC preview.

No resizing or generated replacement art. A manifest fixes source identity,
...
ART=ROOT/'gba/art/johto-cast-v1'
p=argparse.ArgumentParser();p.add_argument('game',type=Path);a=p.parse_args();game=a.game.resolve()
assert game==ROOT/'tools/vendor/gba/johto-cast-v1-game', 'Use the isolated NPC preview.'
...
def section(path,body):
    start='// BEGIN JOHTO CAST IMPORT';end='// END JOHTO CAST IMPORT'
...
for s in spec['assets']:
    ...
    dest=game/'graphics/object_events/pics/people/johto';dest.mkdir(exist_ok=True)
    atlas.save(dest/(name.lower()+'.png'),transparency=0)
    write('graphics/object_events/palettes/johto_'+name.lower()+'.pal',paltext)
    gfx += [f'const u32 gObjectEventPic_{symbol}[] = INCGFX_U32("graphics/object_events/pics/people/johto/{name.lower()}.png", ".4bpp", "-mwidth 4 -mheight 4");',f'const u16 gObjectEventPal_{symbol}[] = INCGFX_U16("graphics/object_events/palettes/johto_{name.lower()}.pal", ".gbapal");']
    ...
    .tileTag = TAG_NONE, .paletteTag = OBJ_EVENT_PAL_TAG_JOHTO_{name.upper()},
    ...
    constants += [f'#define OBJ_EVENT_GFX_JOHTO_{name.upper()} {s["graphics_id"]}',f'#define OBJ_EVENT_PAL_TAG_JOHTO_{name.upper()} 0x{s["palette_tag"]:04X}']
    externs += [f'extern const struct ObjectEventGraphicsInfo gObjectEventGraphicsInfo_{symbol};']
    pointers += [f'    [OBJ_EVENT_GFX_JOHTO_{name.upper()}] = &gObjectEventGraphicsInfo_{symbol},']
    pals += [f'    {{gObjectEventPal_{symbol}, OBJ_EVENT_PAL_TAG_JOHTO_{name.upper()}}},']
...
anims=[];table=[]
for direction,(idle,left,right) in spec['directions'].items():
    for kind,duration in [('FACE',16),('GO',8),('GO_FAST',4),('GO_FASTER',2),('GO_FASTEST',1)]:
        name='sJohtoCast_'+kind+'_'+direction
        frames=[idle] if kind=='FACE' else [left,idle,right,idle]
        anims += [f'static const union AnimCmd {name}[] = {{ '+', '.join(f'ANIMCMD_FRAME({i}, {duration})' for i in frames)+', ANIMCMD_JUMP(0) };']
        table += [f'    [ANIM_STD_{kind}_{direction}] = {name},']
anims += ['static const union AnimCmd *const sAnimTable_JohtoCast[] = {\n'+'\n'.join(table)+'\n};']
for file,body in [('object_event_graphics.h',gfx),('object_event_pic_tables.h',pics),('object_event_anims.h',anims),('object_event_graphics_info.h',infos)]:section('src/data/object_events/'+file,'\n'.join(body))
constants += ['#define OBJ_EVENT_GFX_CAST_FIRST 1029','#define OBJ_EVENT_GFX_CAST_END 1032',
              '_Static_assert(OBJ_EVENT_GFX_VAR_F < OBJ_EVENT_GFX_CAST_FIRST, "Johto graphics overlap dynamic IDs");',
              '_Static_assert(OBJ_EVENT_GFX_CAST_END < OBJ_EVENT_MON, "Johto graphics overlap follower IDs");']
...
replace(path,'const struct ObjectEventGraphicsInfo *const gObjectEventGraphicsInfoPointers[OBJ_EVENT_GFX_JOHTO_END] = {',
    '\n'.join(externs)+'\nconst struct ObjectEventGraphicsInfo *const gObjectEventGraphicsInfoPointers[OBJ_EVENT_GFX_CAST_END] = {\n'+'\n'.join(pointers))
...
replace('src/event_object_movement.c','    if (graphicsId >= NUM_OBJ_EVENT_GFX)',
        '    if (graphicsId >= OBJ_EVENT_GFX_CAST_FIRST && graphicsId < OBJ_EVENT_GFX_CAST_END)\n        return gObjectEventGraphicsInfoPointers[graphicsId];\n\n    if (graphicsId >= NUM_OBJ_EVENT_GFX)')
section('src/event_object_movement.c','\n'.join(constants[-2:]))

# Emerald's stop/turn code only seeks an idle frame for registered step tables.
# Register both the existing regional residents and the custom cast.
replace('src/data/object_events/object_event_anims.h',
    'static const struct StepAnimTable sStepAnimTables[] = {',
    'static const union AnimCmd *const sAnimTable_JohtoHuman[20];\n'
    'static const union AnimCmd *const sAnimTable_JohtoCast[20];\n'
    'static const struct StepAnimTable sStepAnimTables[] = {\n'
    '    {.anims = sAnimTable_JohtoHuman, .animPos = {1, 3, 0, 2}},\n'
    '    {.anims = sAnimTable_JohtoCast, .animPos = {1, 3, 0, 2}},')
...
for placement in spec['placements']:
    ...
    event['graphics_id']='OBJ_EVENT_GFX_JOHTO_'+placement['asset'].upper()
...
board=Image.new('RGB',(800,380),'#15262b');d=ImageDraw.Draw(board)
d.text((12,12),'JOHTO CAST IMPORT / actual converted pixels, 2x; S / N / W / E',fill='white')
for row,s in enumerate(spec['assets']):
    im=Image.open(native/(s['name'].lower()+'.png')).convert('RGBA');y=35+row*66
    d.text((12,y+20),s['name'],fill='white')
    for col,values in enumerate(spec['directions'].values()):
        frame=im.crop((0,values[0]*32,32,values[0]*32+32)).resize((64,64),Image.Resampling.NEAREST)
        board.paste(frame,(100+col*90,y),frame)
d.text((490,80),'Prepared native custom sprites',fill='white')
d.text((490,105),'Fixed anchor applied in Aseprite',fill='white')
d.text((490,130),'Independent east-facing frames',fill='white')
board.save(evidence/'imported-directions.png')
print(json.dumps({'assets':len(assets),'frames':len(assets)*12,'exact_pixels':True,'map_events':sum(len(m['object_events']) for m in maps.values())}))
