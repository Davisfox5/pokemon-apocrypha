...
int main(int argc,char**argv){
 if(argc!=4)return 2;argv_out=argv[3];FILE*f=fopen(argv[2],"r");if(!f)return 3;
 ...
 mCoreConfigInit(&c->config,"johto-restart");c->setVideoBuffer(c,pixels,240);if(!mCoreLoadFile(c,argv[1]))return 5;
 c->rtc.override=RTC_FIXED;c->rtc.value=1789315200000LL;c->reset(c);frames(600,0);call("MapProof_Boot",0);frames(240,0);
 const unsigned views[][2]={{39,17},{48,9},{53,25},{30,20},{36,9}};
 for(view=0;view<5;view++){
  call("MapProof_Enter",(views[view][0]<<8)|(views[view][1]<<16));frames(360,0);
  for(unsigned i=0;i<360;i++){frames(4,0);sample(i*4);char tag[80];sprintf(tag,"view-%u-%03u",view,i);screenshot(argv[3],tag);}
 }
 interaction(1029,40,19,"gold-dialogue");interaction(1030,49,25,"silver-dialogue");interaction(1031,37,23,"kestra-dialogue");
 interaction(1032,48,13,"hoenn-dialogue");interaction(1033,58,19,"kanto-dialogue");interaction(1034,39,13,"johto-dialogue");
 interaction(1035,30,23,"sinnoh-dialogue");interaction(1036,54,30,"unova-dialogue");
 c->deinit(c);return 0;
}
